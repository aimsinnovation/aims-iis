...
namespace Aims.IISAgent.Pipes.Tools
{
	...
	{
		private static readonly Dictionary<Type, Type> EnumlessProxies = new Dictionary<Type, Type>();
		...
		{
			lock (EnumlessProxies)
			{
				Type enumlessType;
				if (EnumlessProxies.TryGetValue(type, out enumlessType))
					return enumlessType;

				enumlessType = CreateEnumlessProxy(type);
				EnumlessProxies.Add(type, enumlessType);
				return enumlessType;
			}
		}
